...
OUTPUT_FILE="./scripts/output/events.json"

# Calculate start time and end time
CURRENT_TIME=$(date +"%Y-%m-%d %H:%M:%S") #MIGHT CHANGE THIS
START_TIME=$(date -d "30 days ago" +"%Y-%m-%d %H:%M:%S") #MIGHT CHANGE THIS
...
where 
  ...
  and Origin.time_value >= '$START_TIME'
  and Origin.time_value <= '$CURRENT_TIME';
EOF
)
...
awk -F"\t" 'BEGIN{OFS=","; print "["} {print "{\"publicID\":\""$1"\",\"OT\":\""$2"\",\"latitude_value\":"$3",\"longitude_value\":"$4",\"depth_value\":"$5",\"magnitude_value\":"$6",\"type\":\""$7"\",\"text\":\""$8"\",\"place\":\"\"}"} END{print "]"}' temp.txt \
  > "$OUTPUT_FILE"
